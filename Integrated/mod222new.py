    ...
    while True:
        ...
        if var_option == "CUST_EMAIL":
            ...
            if len(str(var_phone)) != 10:
                ...
                continue
            pd_result.loc[0, 'CUST_PHONE'] = '('+str(var_phone)[0:3] + ')' + str(var_phone)[
                3:6] + '-' + str(var_phone)[6:]
            ...
            var_zip = pyip.inputInt("Enter new ZIPCODE : ")
            if ((len(str(var_zip)) in range(4, 6))):

                pd_result.loc[0, 'CUST_ZIP'] = var_zip
                print(pd_result.loc[0, 'CUST_ZIP'])
            else:
                print("Invalid:Pass 4 or 5 digit valid zipcode")
                continue
            pd_result.loc[0, 'CUST_ZIP'] = var_zip
            print(pd_result.loc[0, 'CUST_ZIP'])
...
CUST_STATE = '{}', CUST_ZIP = {} WHERE SSN = {}".format(pd_result.loc[0, 'FIRST_NAME'],
                                                        pd_result.loc[0, 'MIDDLE_NAME'], pd_result.loc[0,
                                                                                                       'LAST_NAME'], pd_result.loc[0, 'CUST_EMAIL'],
                                                        pd_result.loc[0, 'CUST_PHONE'], pd_result.loc[0,
                                                                                                      'FULL_STREET_ADDRESS'], pd_result.loc[0, 'CUST_CITY'],
...
# def update_cust_details(df_sp_cust, list_ssn):
#     while True:
        # var_ans = pyip.inputStr(
        #     "Would you like to update Customer Information? (Y/N) : ")
        # var_ans
