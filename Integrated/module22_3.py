...
def show_info(df_sp_cc, var_cc, var_month, var_year):
    ...
    result_cc = df_sp_cc.where((df_sp_cc['CUST_CC_NO'] == var_cc) &
                               (df_sp_cc['TIMEID'].substr(5, 2) ==
                                str(var_month).rjust(2, '0')) &
                               (df_sp_cc['TIMEID'].substr(1, 4) == str(var_year)))
    # #result_total = df_sp_cc.where((df_sp_cc['CUST_CC_NO'] == var_cc) &
    #                               (df_sp_cc['TIMEID'].substr(5, 2) ==
    #                                str(var_month).rjust(2, '0')) &
    #                               (df_sp_cc['TIMEID'].substr(1, 4) == str(var_year))) \
    #     .agg(sum(df_sp_cc['TRANSACTION_VALUE']).alias('TOTAL'))
...
def test_call3(df_sp_cc, list_cc):
    while True:
        var_cc = pyip.inputStr(
            "Enter 16-digit Creditcard Number (0 to Exit) : ")
